...
> = (arc) => {
  return function arcInterpolation(this: SVGPathElement, datum) {
    const fromAngles = this["_prior"]
      ? (this["_prior"] as typeof datum)
      : datum;
...
const INTERPOLATE_LINE: InterpolatorFactory<Line<ChartPoint>, LineData> = (
  line_er
) => {
...
const INTERPOLATE_POINT: InterpolatorFactory<
